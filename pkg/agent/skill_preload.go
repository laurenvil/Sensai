...
var skillPreloadRules = []skillPreloadRule{
	...
	// students ask about headers by name or by role.
	{
		pattern: regexp.MustCompile(
...
//
// Setting SENSAI_DISABLE_PRELOAD=1 disables the pre-router entirely. Used by
...
func (cb *ContextBuilder) PreloadSkillsForMessage(message string) string {
	...
	if os.Getenv("SENSAI_DISABLE_PRELOAD") == "1" {
		return ""
	}
