...
// that holds the two Sensai skills with sentinel bodies so we can grep for them
...
func TestPreloadSkillsForMessage_LEDMatrixScroll(t *testing.T) {
	cb := setupPreloadTest(t)
	out := cb.PreloadSkillsForMessage("Scroll 'Sensai' across the LED matrix.")
...
func TestPreloadSkillsForMessage_EnvDisablesPreroute(t *testing.T) {
	cb := setupPreloadTest(t)
	t.Setenv("SENSAI_DISABLE_PRELOAD", "1")
	out := cb.PreloadSkillsForMessage("Make the LED on pin 9 breathe — fade in and out smoothly.")
	if out != "" {
		t.Fatalf("expected empty output when SENSAI_DISABLE_PRELOAD=1, got %d chars", len(out))
	}
}
