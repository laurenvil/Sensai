...
How Sensai turns a student's natural language question into working Arduino code on the Uno Q board, then compiles and flashes that code to the microcontroller. This document is for both developers working on Sensai and for students who want to understand how the system works under the hood.

**v3 architecture summary:** Sensai ships two execution paths sharing the same model, system prompt, and 15-skill tree:
- **Agentic** — agent loop + 23-rule pre-router + **8 tools** (`read_file`, `write_file`, `list_dir`, `arduino`, `camera`, `sysfs_led`, `network`, `i2cdetect`). End-to-end compile/flash, camera capture, MPU LED control, network diagnostics, I²C bus scan. `make sensai-agentic` / `make sensai`.
- **Direct** — same 23-rule pre-router + single LLM call, no tools, no loop. Faster Q&A across all 15 skills; cannot perform hardware actions. `make sensai-direct`.
...
The full evaluation rationale lives in `docs/Sensai/eval/v3/sensai-v3-whitepaper.md`. The capability gap analysis that motivated the 7 new skills and 4 new tools is in `docs/Sensai/eval/v3/sensai-v3-capability-gap-analysis.md`; the implementation record is in `docs/Sensai/eval/v3/sensai-v3-capability-integration.md`.
...
void loop() {
    ...
    matrix.print("      Sensai      "); // space-pad to start/end off-screen
...
The pre-installed `/usr/local/bin/arduino-flash` wrapper hardcodes `0x80F0000` and silently writes sketches into the reserved area, where they never execute. Sensai's arduino tool invokes OpenOCD directly at `0x8100000` to avoid this. Root-cause analysis: `docs/Sensai/eval/v3/sensai-v3-whitepaper.md` §8.

---

## Part 3: How Sensai Works — The Two Data Paths
...
### Path A — Agentic (`make sensai-agentic`)

When a student sends a Telegram message OR types at the agent terminal:

```
Student input (Telegram / terminal / SSH)
    │
    ▼
picoclaw channel adapter (pkg/channels/{telegram,cli,...}.go)
    │ For Telegram: sends "Asking Sensai..." immediately
...
(pkg/agent/skill_preload.go)
    ...
    │ Example: "scroll Sensai across the LED matrix" fires the led-matrix
...
ContextBuilder.BuildMessages() (pkg/agent/context.go)
    ...
    │   2. SOUL.md — Sensai Arduino persona + minimal rules
...
AgentLoop publishes OutboundMessage to bus
    ▼
Channel adapter sends message back to student
```
...
```
Student types in terminal REPL
    │
    ▼
scripts/sensai-direct-chat.py
    │
    ▼
...
The direct path **cannot** compile, upload, or call any tool. Sketches come back as text in the response; the student copies them and uploads via the Arduino IDE (or switches to the agentic path).
...
**Student:** "Use the arduino tool to upload a blink sketch for pin 13 with 500 ms toggles."
...
6. Tool returns `"Sketch compiled and flashed to the board."` — LLM iteration 2 returns final confirmation to the student.
...
5. LED matrix begins scrolling "Sensai" in blue within a few seconds of the flash completing.
...
**OpenCL prefill acceleration for llama-server (planned, tracked under `docs/Sensai/Vulkan/`):**
```bash
...
### Agentic path (`make sensai-agentic`)
...
### Direct path (`make sensai-direct`)

| Capability | How |
|-----------|-----|
| Generates Arduino sketches | LLM text generation, with pre-router-loaded canonical templates (returned as text — student copies into the Arduino IDE or App Lab) |
...
### What Sensai Cannot Do (either path)
...
The v3 config (`config/sensai.config.json`) disables these tools that were available in v2:

| Tool | Reason for v3 removal |
|------|----------------------|
| `exec` | Security risk; educational scope doesn't need shell |
| `i2c` | Hardware-specific; ~400 chars of tool schema saved |
| `spi` | Hardware-specific; ~400 chars of tool schema saved |
| `message` | Inter-agent messaging; not used in educational context |
...
./yzma/lib/llama-server \
  ...
  --parallel 1 \           # single-slot — students queue if multiple arrive
...
- `--parallel 1`: prevents two students' prompts from interleaving in the KV cache. For multi-student classrooms, run multiple Uno Qs or accept the queue.
...
│   ├── AGENTS.md                (if exists)
│   ├── SOUL.md                  ← Sensai Arduino persona + minimal rules
│   ├── USER.md                  (if exists)
...
**Direct path mirror:** `scripts/sensai-direct-chat.py` ports the same 11 rules to Python and assembles a minimal system prompt (SOUL.md + pre-router only — no skills XML, no tool schema).

---

## Part 8: Student Mental Model

For students who want to understand the system at a high level. The picture depends on which mode the teacher is running.
...
You type a question (e.g. "upload a blink sketch for D13")
        │
        ▼
Sensai's pre-router scans your question for keywords:
...
— no internet needed —
        │
        ▼
Sensai writes an Arduino sketch AND
...
You type a question (e.g. "which pins do PWM?")
        │
        ▼
Sensai's pre-router scans for keywords:
...
1. **Linux processor (MPU, the QRB2210):** running Sensai itself — the AI model, the pre-router, the agent loop (in agentic mode), and the optional Telegram gateway. Uses ~1.3 GB of RAM.
...
├── scripts/
│   ├── sensai-launch.sh         # Agentic path launcher (loop + tools)
│   ├── sensai-launch-direct.sh  # Direct path launcher (pre-router + REPL)
│   ├── sensai-direct-chat.py    # ← Direct path Python REPL
...
├── docs/Sensai/                 # Guides, whitepapers, evaluation artifacts
│   ├── teacher-guide.md
│   ├── student-guide.md
│   ├── development/             # This document, setup walkthrough, etc.
│   └── eval/v3/                 # ← 8 evaluation runs + whitepaper + gap analysis + integration record
│       ├── sensai-v3-whitepaper.md
│       ├── sensai-eval-v3-run7.md  # agentic-path evaluation
│       ├── sensai-eval-v3-run8.md  # direct-path evaluation
│       └── Artifacts/Run [1-8]/    # raw cell transcripts and server logs
└── Makefile                     # make sensai-agentic, sensai-direct, ...
```
