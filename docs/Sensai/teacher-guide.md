...
Sensai has **two ways for students to interact** AND **two execution modes** (new in v3) — pick the right mode for the lesson goal.

### Interaction channels

1. **Terminal** — connect a keyboard and monitor (or SSH) and type questions at the `You:` prompt. Sensai prints its response directly below. Launched by `make sensai-agentic` or `make sensai-direct`.
2. **Telegram** — send messages from any phone or laptop on the same network (or internet with port forwarding). Telegram is **only available in agentic mode** (`make sensai-agentic`).

### Execution modes (new in v3)

| Mode | Command | Best for | Latency |
|---|---|---|---|
| **Agentic** | `make sensai-agentic` (or `make sensai`) | Lessons that end with "and upload it to my board" — Sensai can actually flash the sketch | ~12–22 min/turn |
| **Direct** | `make sensai-direct` | Q&A about pinouts, voltage rules, concepts; short sketches students will copy by hand | ~5–13 min/turn |

**How to choose:**

- "We're learning what the pins do" → **direct**
- "We're learning how to write a blink sketch" → either; direct is faster, agentic adds end-to-end compilation
- "We're learning how to make the LED matrix scroll our name" → **agentic** (it can flash the board for you)
- "Multiple students querying one board for quick questions" → **direct** (lower per-turn latency, more throughput)
- "Telegram bot for after-school office hours" → **agentic** (only mode with Telegram)

You can switch modes between sessions. Both share the same workspace, system prompt, and skill files; the only difference is whether the agent loop and the `arduino` tool are active. Use `make sensai-stop` to clean up before switching.
...
Pick the mode that fits your lesson. Both can be run from the same install — they share workspace and config.

```bash
make sensai-agentic    # default — agent loop + arduino tool (compile/upload)
# or
make sensai-direct     # fast Q&A — no compile/upload, ~33% lower latency
```

`make sensai` is an alias for `make sensai-agentic` to preserve the v2 muscle memory.
...
# Default — agentic path (loop + tools + compile/upload)
make sensai

# Alternative — direct path (faster, Q&A only)
make sensai-direct
```

That is the only command you need each session. It starts the inference engine, opens the terminal chat, and (in agentic mode) starts the Telegram gateway if a token is configured. Press `Ctrl+C` to stop everything cleanly.
...
|---|---|
| `make sensai` | Start Sensai for the session (alias for `sensai-agentic`) |
| `make sensai-agentic` | Agentic path: loop + 4 tools + Telegram (compile/upload workflows) |
| `make sensai-direct` | Direct path: pre-router only, no tools (fast Q&A) |
| `make sensai-onboard` | Re-run the setup wizard (change Telegram token, allow list) |
| `make sensai-setup` | Reinstall the system prompt and skills tree after a git pull |
...
## Sketch Compilation and Upload (Agentic Mode Only)

In **agentic mode** (`make sensai-agentic`), Sensai can compile and upload Arduino sketches directly to the board. When a student asks Sensai to write a sketch, Sensai can verify it compiles — and if a board is connected, upload it automatically via OpenOCD at the correct sketch partition address (`0x8100000`). The student never leaves the chat.

**This is NOT available in direct mode** — direct mode returns the sketch as text for the student to copy and upload manually.

**Prerequisites:** `arduino-cli`, OpenOCD, and the Uno Q board core are installed automatically as part of `make sensai-install`. If you need to install or reinstall them separately:

```bash
make sensai-arduino-setup
```

Once installed, Sensai detects the tool automatically. To reliably trigger the `arduino` tool, students should **name the tool explicitly** in the prompt:

> "**Use the arduino tool to upload** a blink sketch for D9 to the board" ✅

> "Compile this code and tell me if there are errors" — works if `compile` keyword is present

> "What boards are connected?" — runs `arduino-cli board list`

**Why directive prompts work better:** v3 evaluation found that at 0.8B scale, ambient prompts like "and upload it to the board" sometimes produce a correct sketch in markdown rather than triggering the tool. Naming the tool explicitly ("Use the arduino tool to...") raises tool-call reliability to ~100%. The student-guide includes phrasing tips for this.

Sensai will compile the sketch, report any errors in plain language, fix them, and upload when asked. The LED matrix and GPIO pins respond within seconds of the flash completing.
...
**Expected response times (Qwen3.5-0.8B-Q4_0 on Uno Q, v3 measured):**

These are the actual walltimes measured during the v3 evaluation runs (cold-prefill + decode, fresh server, full 20K-char system prompt with pre-router skill content). Subsequent turns in the same session are faster because the KV cache is warm.

| Prompt type | Direct path | Agentic path |
|---|---|---|
| Factual ("Which pins do PWM?") | ~5–8 min | ~6 min |
| Concept ("MPU vs MCU?") | ~5–6 min | ~8 min |
| Voltage safety ("Can I connect 5V to A0?") | ~9 min | ~10 min |
| Short sketch (blink, ~20 lines) | ~6 min | ~10 min |
| Full sketch (breathing, ~30 lines) | ~11 min | ~13 min |
| Sketch + upload (LED matrix scroll) | ~13 min (text only) | ~20 min (with flash) |

The agentic path is consistently slower because the agent loop iterates (e.g. explore → write_file → arduino tool) and each tool call costs a fresh prefill. The direct path is a single shot.

Set student expectations:

- Direct mode: *"Sensai needs about 5–10 minutes — give it space to think."*
- Agentic mode: *"Sensai is going to write, compile, and flash this — about 15–20 minutes. Watch the matrix when it finishes."*

For impatient classes, **direct mode is the better default** unless the lesson specifically needs end-to-end automation.
