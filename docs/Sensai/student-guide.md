...
## Two Modes — Ask Your Teacher Which One Is Running

Your teacher may launch Sensai in one of two ways:

- **Agentic mode** (`make sensai`) — Sensai can write a sketch AND upload it to your board automatically. Slower (about 15–20 minutes for a full sketch + upload), but you don't have to copy the code anywhere.
- **Direct mode** (`make sensai-direct`) — Sensai answers questions and writes sketches as text. You copy the code into the Arduino IDE and upload it yourself. Faster (about 5–10 minutes per answer), good for quick questions.

You won't see a difference in how you type — both look identical. The difference is what Sensai can do *after* writing the code.

If you want Sensai to actually flash a sketch to your board, **say it out loud**: "Use the arduino tool to upload..." If you just want to see the code, ask normally.

---

## How to Talk to Sensai

You have three ways to reach Sensai:

**Option 1 — Terminal (directly on the board)**

Once your teacher runs `make sensai` (or `make sensai-direct`), the terminal shows a welcome banner and then a `You:` prompt. Type your question and press Enter. Sensai thinks for a few seconds (or minutes for a long sketch), then prints its answer right below.
...
**Sensai can:**
- **Write, compile, and upload sketches directly to your board** (agentic mode only). Just ask — and say "**use the arduino tool**" for best results: *"Use the arduino tool to upload a blink sketch for D9."* Sensai will generate the code, compile it, and flash it. If there is a compile error, it reads the output, explains what went wrong, fixes it, and tries again.
- **Detect which boards are connected** (agentic mode) — ask "What boards are connected?" and Sensai will check.
- **Explain errors and fix broken code** — paste an error message and your code, and Sensai diagnoses the problem.
- **Answer factual questions about the board** in any mode — pinouts, voltage rules, why a function works the way it does.

**Sensai cannot:**
- **Upload sketches in direct mode.** Direct mode (`make sensai-direct`) gives you the code as text; you copy and upload it yourself. If your teacher is running direct mode and you want to flash from the chat, ask them to switch to agentic mode.
...
- **Answer instantly.** Sensai thinks on the board's processor, not a data center. Short answers arrive in about 5–10 minutes. Full sketches take 10–20 minutes. That is normal — be patient. (If you are running direct mode and the answer is a one-liner, it can be faster.)
