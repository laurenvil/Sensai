#!/usr/bin/env python3
"""Sensai Direct-Server interactive REPL — pre-router + direct API, no agent loop, no tools.

Mirrors the Direct evaluation architecture (eval_v8_direct_preroute.py) but in
an interactive chat form for classroom use.
...
Use this path when:
  - The student is asking factual / conceptual questions
...
instead: `make sensai` (or `make sensai-agentic`), which exposes 8 narrow tools
...
DEFAULT_TEMP = float(os.environ.get("SENSAI_TEMP", "0.3"))
MAX_TOKENS = int(os.environ.get("SENSAI_MAX_TOKENS", "2048"))
REQUEST_TIMEOUT_SECS = int(os.environ.get("SENSAI_REQUEST_TIMEOUT", "1800"))
...
BANNER = """
  ...
  │  run `make sensai-agentic` instead.            │
  │                                                │
...
def main() -> int:
    if not SOUL_PATH.is_file():
        ...
        print("Run: make sensai-setup", file=sys.stderr)
        return 2
    model = server_model_id()
    if model is None:
        print("Error: llama-server not reachable at 127.0.0.1:8080", file=sys.stderr)
        print("Start it with: make sensai-direct  (or start sensai-launch-direct.sh)", file=sys.stderr)
        return 2

    print(BANNER)
    print(f"Sensai is ready — model: {model}  (temp={DEFAULT_TEMP})")
