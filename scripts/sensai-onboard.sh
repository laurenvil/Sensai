#!/usr/bin/env bash
# sensai-onboard.sh — First-run setup for Sensai on Arduino Uno Q.
# Patches only the Sensai-relevant fields in config.json (token, allow_from).
...
echo ""
echo -e "${BOLD}  Sensai — Arduino AI Assistant  ${RESET}"
echo -e "${DIM}  First-run setup for Arduino Uno Q${RESET}"
hr
echo ""

# ── Check config exists (sensai-setup must have run first) ────────────────────

if [ ! -f "$CONFIG" ]; then
    echo -e "${YELLOW}Config not found. Running sensai-setup first...${RESET}"
    make sensai-setup
    echo ""
fi
...
if [ -f "$SENSAI_MODEL" ]; then
    ...
    echo -e "  ${YELLOW}Model not found at: $SENSAI_MODEL${RESET}"
    echo ""
    echo "  Download it now with:"
    echo -e "  ${CYAN}  mkdir -p ~/models"
    echo -e "  ${CYAN}  wget -O $SENSAI_MODEL \\"
    echo -e "  ${CYAN}    'https://huggingface.co/Qwen/Qwen3.5-0.8B-GGUF/resolve/main/Qwen3.5-0.8B-Q6_K.gguf'${RESET}"
    echo ""
    echo "  (You can still continue setup — download the model before running 'make sensai')"
fi
echo ""
...
if [ -f "$LLAMA_SERVER" ]; then
    ...
    echo ""
    echo "  (You can still continue setup — download llama-server before running 'make sensai')"
fi
echo ""
...
if [ -n "$TOKEN_INPUT" ]; then
    ...
    echo -e "  Or press Enter to ${BOLD}allow anyone${RESET} (fine for a classroom on a local network)"
    printf "  Allow from: "
    read -r ALLOW_INPUT
...
PYEOF
    ...
    echo -e "  ${DIM}Skipped — Sensai will run as terminal-only.${RESET}"
    echo "  Run 'make sensai-onboard' at any time to add Telegram later."
fi
...
hr
echo ""
echo -e "${GREEN}${BOLD}Sensai is ready.${RESET}"
echo ""
