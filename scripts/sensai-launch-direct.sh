#!/usr/bin/env bash
# sensai-launch-direct.sh — Start llama-server (background) and drop into the
...
# come back as text for the student to copy.
#
...
# compile/upload/camera/LED/I²C workflows use `make sensai-agentic` instead.
set -euo pipefail
...
DIRECT_CHAT="${DIRECT_CHAT:-./scripts/sensai-direct-chat.py}"
...
cleanup() {
    echo ""
    echo "Stopping Sensai (direct mode)..."
