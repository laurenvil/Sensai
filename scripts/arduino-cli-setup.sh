...
echo -e "${DIM}  Compile and upload sketches from Sensai${RESET}"
hr
echo ""
...
if [ -n "$ARDUINO_CLI_PATH" ]; then
    ...
    if command -v curl > /dev/null 2>&1; then
        ...
        echo "  Install one and re-run: make sensai-arduino-setup"
        exit 1
    fi
...
echo "  Sensai will compile and upload sketches automatically."
echo ""
