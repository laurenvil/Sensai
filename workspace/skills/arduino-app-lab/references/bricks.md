...
A **Brick** is a pre-packaged Linux-side service that drops into an App Lab project. Bricks encapsulate common building blocks so a student doesn't have to write the underlying infrastructure.
...
Sensai does NOT install or manage Bricks. Sensai writes sketches and Python code; App Lab is where Bricks are added.

What Sensai *can* do is generate code that **uses** a Brick once it's been installed in your App. The skill content available to Sensai includes patterns for the standard Brick APIs.
...
- **Heavy-weight Bricks on the 2 GB Uno Q variant.** Some AI Bricks need ~600 MB of RAM. On the 2 GB board, RAM gets tight if you also run `llama-server` for Sensai. Prefer the 4 GB variant for production AI projects.
