...
2. **Flash the MCU** via OpenOCD at sketch partition `0x8100000`. (Same path Sensai's `arduino` tool uses.)
...
## How this relates to Sensai

Sensai's `arduino` tool does steps 1+2 (compile + flash) for a single sketch. It does NOT do steps 3–5 (multi-file Apps, Bricks, Python deployment).

A student can use Sensai to iterate quickly on the sketch portion, then move into App Lab when they need the full App workflow (Python integration, Bricks, multiple files).
...
- `sketch-patterns/references/upload.md` for the equivalent Sensai-side flow on the sketch portion only.
