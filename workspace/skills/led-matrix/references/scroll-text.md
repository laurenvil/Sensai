...
| `matrix.print("      Sensai      ")` | The text. **Pad with spaces** — without them the scroll starts and ends mid-character. The matrix is 13 columns wide and `Sensai` is ~30 columns of pixels at Font_5x7, so padding is what makes the scroll readable. |
