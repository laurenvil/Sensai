...
4. **Wrap text in spaces:** `"   Sensai   "` — without leading/trailing spaces the scroll starts and ends mid-character. The matrix is only 13 columns wide, so a 6-character word at Font_5x7 (~30 columns of pixels) already overflows; the scroll is the point.
...
- `references/scroll-text.md` — Canonical template for scrolling text across the matrix (the "Sensai" demo).
