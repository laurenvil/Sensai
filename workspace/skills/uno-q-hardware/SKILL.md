...
Sketches always target the MCU. The MPU is where Sensai itself runs.

## When to load the references

- Student asks about a specific pin (D-number, A-number) → read `references/pinout.md`.
- Student asks about voltage, 5 V sensors, ADC range, or "is it safe" → read `references/voltage-safety.md`.
...
| `references/voltage-safety.md` | 3.3 V vs 5 V rules, MPU↔MCU I/O split, common ways students damage the board. |
