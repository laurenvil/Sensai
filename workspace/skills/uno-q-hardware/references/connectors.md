...
→ Linux owns the SE0 bank on **JMISC** (1.8 V) and the JCTL GPIOs (1.8 V). These can be driven via `/sys/class/gpio` or `libgpiod`. Note the **1.8 V level** — you almost always need a level shifter if you're connecting a 3.3 V or 5 V external component. Most student projects should drive headers from the MCU instead, where 3.3 V is native.
