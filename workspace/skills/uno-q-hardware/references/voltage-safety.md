...
**Sketches see only the 3.3 V MCU world.** When a student says "pin", "Wire", or "analogRead", that is 3.3 V.

## The 5 V trap

Classic Uno is 5 V tolerant on I/O. **Uno Q is not.** Common student mistakes:
...
A reading of `1023` means 3.3 V, not 5 V. If a student is "expecting more range," explain the 3.3 V ceiling.
...
- When a student asks something that requires Linux (file I/O, network requests, OpenCV), it is an MPU/Python question — say so.
