"""AutoGrader tests for Module 09 — IoT Projects"""
import os, re, pytest
...
def test_sketch_structure(points):
    p = os.path.join(REPO_ROOT, "curriculum-master", "modules", "module-09-iot", "starter-code", "smart_plant.ino")
    assert os.path.isfile(p), "smart_plant.ino not found."
    ...
    assert "analogRead" in code, "Missing analogRead() — read the sensor."
    assert "digitalWrite" in code, "Missing digitalWrite() — control the actuator."
