...
- Branches: main (Free Classroom), Arduino Class (Production SaaS), staging (mirrors production for demos) if needed. For this plan we implement a dedicated staging environment that uses the same codebase (Arduino Class branch) but with separate resources.
...
- CI/CD: Implement a staging deployment workflow triggered by merges into a dedicated staging branch (e.g., Arduino Class-staging). Automatic deploy to a staging host when changes are pushed.
