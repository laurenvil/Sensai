# AI Integration Architecture — Sensai Classes

Sensai Classes uses **GitHub Models** — GitHub's built-in AI inference API — to power all AI features. No external dependencies, no API keys, no third-party services.
...
- **Interactive simulations** are its strength, but Sensai Classes focuses on text/code/quiz content
...
In Sensai Classes, AI is not a black box—it is infrastructure that students can improve.
